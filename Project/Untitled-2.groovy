...
#include <string>  // For using the string class to represent plugin names

using namespace std; // Avoids repeatedly writing std:: before standard library elements

class PluginManager {
private:
    vector<string> plugins;  // List of available plugins - a dynamic array of strings

public:
    // Function to add a new plugin to the list
    void addPlugin(const string& pluginName) { // Takes the plugin name as input (passed by const reference for efficiency)
        plugins.push_back(pluginName); // Adds the plugin name to the end of the plugins vector
        cout << "\n✅ Plugin '" << pluginName << "' added successfully!" << endl; // Prints a success message
    }

    // Function to list all available plugins (display them)
    void listPlugins() {
        if (plugins.empty()) { // Checks if the plugins vector is empty
        ...
        for (size_t i = 0; i < plugins.size(); ++i) { // Loops through the plugins vector using an index (size_t is an unsigned integer type)
            cout << i + 1 << ". " << plugins[i] << endl; // Prints the index (starting from 1) and the plugin name
        }
    }
    ...
    void loadPlugin(int pluginIndex) { // Takes the index (position) of the plugin to load as input
        ...
        cout << "\n🔊 Applying '" << plugins[pluginIndex - 1] << "' effect..." << endl; // Prints a message simulating loading the plugin effect
    }
...
int main() {
    ...
    string pluginName;    // Variable to store the name of a plugin
